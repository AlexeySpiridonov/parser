...
class angel_co
{
            ...
            if (isset($page['ids'])) {
                foreach ($page['ids'] as $id) {
                    ...
                    if (!$this->db->checkURL($url)) {
                        ...
                        if (!$site)
                            $site = isset ($page['startup_roles'][0]['startup']['company_url']) ? $page['startup_roles'][0]['startup']['company_url'] : false;
    ...
    function site($page)
    {
        ...
        if (preg_match('/\"company_url\":\"(.*?)\"/', $page, $res))
            return preg_replace('/\/', '', $res[1]);
